...
public class TaskBean extends EntityBean<Task, Long> { 
    ...
    public List<Task> findByUsername(String username) {
        return em.createQuery("SELECT t FROM Task t WHERE t.owner.username = :username ORDER BY t.renamecreateDate, t.renamecreateTime")
    ...
    public List<Task> search(String search, Category category, TaskStatus status) {
        ...
        // ORDER BY renamecreateDate, renamecreateTime
        query.orderBy(cb.asc(from.get("renamecreateDate")), cb.asc(from.get("renamecreateTime")));
